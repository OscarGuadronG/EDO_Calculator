...
class MultipasosController:
    ...
    def execute(self, method, f, x0, y0, h, n):
        ...
        if method == "ab2":
            solver = AdamsBashforth2Method(f, x0, y0, h, n)

        elif method == "ab3":
            solver = AdamsBashforth3Method(f, x0, y0, h, n)

        elif method == "ab4":
            solver = AdamsBashforth4Method(f, x0, y0, h, n)

        elif method == "am2":
            solver = AdamsMoulton2Method(f, x0, y0, h, n)

        elif method == "am3":
            solver = AdamsMoulton3Method(f, x0, y0, h, n)

        elif method == "am4":
            solver = AdamsMoulton4Method(f, x0, y0, h, n)

        elif method == "predictor_corrector":
            solver = PredictorCorrectorMethod(f, x0, y0, h, n)
